...
public class MockDataGenerator {
        ...
        try (SqlSession session = sqlSessionFactory.openSession(ExecutorType.BATCH)) {
            ...
            try {
                while (count-- > 0) {
                    ...
                    News newToBeInserted = new News(currentNews.get(index));
                    Instant currentTime = newToBeInserted.getCreatedAt();
                    currentTime = currentTime.minusSeconds(random.nextInt(3600 * 24 * 365));
                    newToBeInserted.setModifiedAt(currentTime);
                    newToBeInserted.setCreatedAt(currentTime);
                    session.insert("com.github.hcsp.MockMapper.insertNews", newToBeInserted);
    ...
    //    private static final int TARGET_
